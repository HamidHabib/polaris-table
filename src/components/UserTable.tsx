...
const demoReducer: React.Reducer<TableState, TableActions> = (
  state,
  action
) => {
  switch (action.type) {
...
function CustomToolbar() {
  ...
  const [state, dispatch] = React.useReducer(demoReducer, TABLE_INITIAL_STATE);
  const { user } = useUser();
  ...
  return (
    <GridToolbarContainer>
      {user && (
        ...
        >
          Load ({Object.keys(state.views).length})
        </Button>
      )}
...
export default function RestoreStateApiRef() {
  ...
  function getIsPrivate(params: any) {
